...
func ParameterDescriptiom(paramsNumber int) []byte {
	...
	param2B := make([]byte, 4)
	binary.BigEndian.PutUint32(param1B, uint32(p2))

	binary.BigEndian.PutUint32(selfMessageLength, uint32(len(paramsNumberB)+len(param1B)+len(param2B)+4))

	return bytes.Join([][]byte{messageType, selfMessageLength, paramsNumberB, param1B, param2B}, nil)
}
